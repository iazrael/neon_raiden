...
export class GameStorage {
    ...
    async recordWeapon(weaponId: WeaponId, damage: number = 0): Promise<boolean> {
        const data = this.getData();
        const stats = data.weapons[weaponId] ?? createDefaultEntityStats();
        const now = Date.now();
    ...
    async recordItem(itemType: BuffType): Promise<boolean> {
        const data = this.getData();
        const stats = data.items[itemType] ?? createDefaultEntityStats();
        const now = Date.now();
    ...
    async recordEnemy(
        ...
        const data = this.getData();
        const stats = data.enemies[enemyId] ?? createDefaultEntityStats();
        const now = Date.now();
    ...
    async recordBoss(
        ...
        const data = this.getData();
        const stats = data.bosses[bossId] ?? createDefaultEntityStats();
        const now = Date.now();
