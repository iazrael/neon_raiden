...
4.  **物理与速度单位**：
    *   **Velocity 组件**：所有速度字段（`vx`, `vy`）使用**像素/秒**作为单位。
    *   **MoveIntent 组件**：当 `type='velocity'` 时，`dx`/`dy` 使用**像素/秒**。
    *   **Knockback 组件**：击退速度使用**像素/秒**。
    *   **SpeedStat 组件**：`maxLinear` 使用**像素/秒**。
    *   **时间单位**：所有系统的 `dt` 参数使用**毫秒**（在物理计算时转换为秒）。
    *   **参考速度值**：
        *   玩家最大速度：400 像素/秒
        *   Boss 最大速度：120 像素/秒
        *   Boss 入场速度：150 像素/秒
