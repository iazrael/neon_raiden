...
当玩家同时持有以下武器对时，自动触发被动/改造效果；默认不启用双弹并发（见下方例外）。
...
| VULCAN | MISSILE | **弹幕覆盖** | 以 VULCAN 为主武器；每次射击同时额外发射 **1级 MISSILE**（导弹依等级1计算，其余数值不变）。 |
| MAGMA | SHURIKEN | **熔火飞刃** | 手里剑命中即附加 `灼烧`（5/秒×3s）。 |
...
### 2.3.3 例外规则说明（MISSILE + VULCAN）
- 主武器固定为 `VULCAN`；武器等级显示与射速计算均以 `VULCAN` 为准。
- 同步发射 `MISSILE(Lv.1)`，不受玩家当前武器等级影响；导弹升级效果不叠加，仅按一级配置取值。
- 该例外不影响其他协同的合成弹/轮发模式设定。
...
**保底掉落（P3新增）**
- 若玩家在一段时间内没有获得任何道具掉落，则触发一次“保底掉落”，在下一次击杀判定中直接生成道具。
- 触发阈值（毫秒）：`EASY=25000`、`NORMAL=30000`、`HARD=20000`。
- 触发后计时重置；Boss 掉落与普通掉落都会重置计时。
...
*   **生成概率**: 基础 10% (受动态难度修正)。
*   **Boss期修正**: Boss 活跃期间小怪仍会少量刷新；刷新间隔乘以 `enemySpawnIntervalInBossMultiplier=1.35`，精英概率乘以系数 `eliteChanceBossMultiplier=0.5`。
...
*   **保底规则**:
    *   动态难度系统维护“无掉落计时器”，超过阈值后在下一次击杀时强制生成一个掉落。
    *   阈值按难度配置，详见“3.2 动态难度系统”。
